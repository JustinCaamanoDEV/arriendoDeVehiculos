        ...
        if($_POST['btnRegistrar']=="REGISTRAR"){
            ...
            $diffDias = $fecha1->diff($fecha2);
                ...
                if($row = mysqli_fetch_array($rs)){
                    ...
                    if(empty($cli) || empty($pat) || empty($fecha1) || empty($fecha2) || empty($usu) || empty($con)){
                        ...
                        $result1 = mysqli_query($cnn,"SELECT usuario FROM cliente WHERE rut='$cli'");
                        $rw1 = mysqli_fetch_array($result1);
                        $usuario = $rw1[0];
                        //Fin de Comparar y buscar tabla Cliente
                        //Comparar y buscar la llave primaria de la tabla Cliente para insertar
                        $result2 = mysqli_query($cnn,"SELECT contraseña FROM cliente WHERE rut='$cli'");
                        $rw2 = mysqli_fetch_array($result2);
                        $contraseña = $rw2[0];
                        //Fin de Comparar y buscar tabla Cliente

                        if(($usuario == $_POST['txtUsu']) && ($contraseña == $_POST['txtCon'])){
                        //Comparar y buscar la llave primaria de la tabla Cliente para insertar
                        ...
                        //Comparar y buscar la llave primaria de la tabla Empleado para insertar
                        $result5 = mysqli_query($cnn,"SELECT rut FROM empleado WHERE rut='18044015-1'");
                        $rw5 = mysqli_fetch_array($result5);
                        $rutE = $rw5[0];
                        //Fin de Comparar y buscar tabla Empleado
                        $sql2 = "UPDATE vehiculo SET estado='A' WHERE patente='$patente' and estado='$estado'";
                        $sql = "INSERT INTO arriendo (rutCliente,rutEmpleado,patente,fechaArriendoWeb,fechaInicioArriendo,fechaFinArriendo,total)VALUES ('$cli','$rutE','$patente','$fecAct','$ini','$fin','$totalFin')";
                        echo "----------------------------------------------------------------------------------------------- <br>";
                        ...
                        <script>alert("El Arriendo del vehículo ha sido ingresado Correctamente")</script><?php
                            ...
                            ?><script>alert("Usuario o Contraseña invalido.")</script><?php
                        }
                    }
        }  
        ?>          
