...
<body onload="startTime()">
    ...
    <table border="0" align="center" style="color: white;">
        <tr>
            <td colspan="4" align="center"><h1>Registro de Empleados</h1></td>
        </tr>
        <tr>
            <td><b>Rut: </b></td>
        ...
        <tr>
            ...
            <td>
                <?php
                    ...
                    $result = mysqli_query($cnn,$sql);
                ?>
                    <select name="txtCar" style="width:160px">
                        <?php echo'<option>'."Seleccione".'</option>';?>
                        <?php echo'<option>'."----------".'</option>';?>
                        <?php while ($row = mysqli_fetch_array($result)){
                            echo'<option>'.$row["descripcion"].'</option>';
            ...
            if($_POST['txtDigito']==$dig){
                ...
                ?><script>alert("Ya existe un cliente registrado con ese Rut")</script><?php
                    ...
                    if(empty($nom) || empty($ape) || empty($fec) || empty($sex) || empty($dir) || empty($reg) || empty($fon) || empty($cor) || empty($car) || empty($usu) || empty($con)){
                        ...
                        $sql = "INSERT INTO empleado VALUES ('$rut','$nom','$ape','$fec','$idSex','$dir','$idReg','$fon','$cor','$idCar','$usu','$con')";
